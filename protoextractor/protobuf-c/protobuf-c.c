...
scan_length_prefixed_data (size_t len, const uint8_t *data, size_t *prefix_len_out)
{
  ...
  *prefix_len_out = hdr_len;
  if (hdr_len + val > len)
    {
      //UNPACK_ERROR (("data too short after length-prefix of %u",val));
      return 0;
    }
  return hdr_len + val;
}
  ...
  while (rem > 0)
    {
      ...
      if (used == 0)
        {
          //UNPACK_ERROR (("error parsing tag/wiretype at offset %u",(unsigned)(at-data)));
      ...
      switch (wire_type)
        ...
        case PROTOBUF_C_WIRE_TYPE_VARINT:
          {
            ...
            if (i == max_len)
              {
                //UNPACK_ERROR (("unterminated varint at offset %u",(unsigned)(at-data)));
                goto error_cleanup_during_scan;
              }
            tmp.len = i + 1;
          }
        ...
        case PROTOBUF_C_WIRE_TYPE_64BIT:
          if (rem < 8)
            {
              //UNPACK_ERROR (("too short after 64bit wiretype at offset %u",(unsigned)(at-data)));
              goto error_cleanup_during_scan;
            }
          tmp.len = 8;
          break;
        ...
        case PROTOBUF_C_WIRE_TYPE_32BIT:
          if (rem < 4)
            {
              //UNPACK_ERROR (("too short after 32bit wiretype at offset %u",(unsigned)(at-data)));
              goto error_cleanup_during_scan;
            }
          tmp.len = 4;
          break;
        default:
          //UNPACK_ERROR (("unsupported tag %u at offset %u",wire_type, (unsigned)(at-data)));
          goto error_cleanup_during_scan;
        }
      ...
      for (j = 0; j < max; j++)
        {
          if (!parse_member (slab + j, rv, allocator))
            {
              //UNPACK_ERROR (("error parsing member %s of %s",slab->field ? slab->field->name : "*unknown-field*", desc->name));
              goto error_cleanup;
            }
        }
    }
